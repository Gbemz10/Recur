...
class HelpCentreScreen extends StatelessWidget {
  ...
  // Built from context rather than a static const list — the answer text
  // used to hardcode AppColors.neutral600, which measures a reasonable
  // ~6:1 contrast against the light surface it was written for but only
  // ~2.7:1 against dark mode's near-black ones, well under WCAG AA's 4.5:1
  // floor. AppColors.muted(context) needs a BuildContext to pick the right
  // tone per theme, which a `static const` field can't provide.
  List<AppAccordionItem> _faqs(BuildContext context) {
    final style = TextStyle(color: AppColors.muted(context), fontSize: 13.5, height: 1.5);
    return [
      AppAccordionItem(
